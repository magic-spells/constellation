...
or at the git root when there is no plan (never `init_plan` just to get it). It is
gitignored and read by the `working_*` tools and `orient` when a plan exists. It is
deliberately not a card type and shares no vocabulary with cards.
...
- `working_list { log? }` — the set; `log: "today"` adds the day's log lines. `orient`
  embeds the same `{header, items}` under `working` when a plan and the folder exist.
