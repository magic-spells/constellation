...
That asymmetry is forced: git names the individual files a commit touched and never the
folder, so a card bound to `tests` would register no drift at all under equality. The
report names the changed files themselves, not the folder — "3 files changed" is the
signal, "tests changed" is not.
